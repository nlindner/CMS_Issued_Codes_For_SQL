...
## CMS_ICD_Code_Descriptions
  ...
  * Contains subdirectories with CMS-issued code descriptions, for both ICD diagnosis and procedure codes, read in via one of the ICD\_Diag_001_Import_Descriptions... SPs or ICD\_Proc_001_Import_Descriptions.... If you do not already have a CMS-based ICD Diagnosis or Procedure code lookup table, this is an example of how to loop through to read in the annual CMS-issued files. 
  * The flat files with the ICD-9 code descriptions available here are reformatted versions of the files I downloaded directly from CMS. I created them by saving the .xls or .xlsx file within the FY release as bar-delimited flat file, with double-quote text qualifiers around all fields in all rows. I did this because of the variations in what was available in the ICD-9 releases as the flat (non-Excel) files. For example, the flat files:
      ...
      * The flat files for the release did not always contain any post-effective-date corrections to the long descriptions (e.g., diagnosis code 86415 for CMS FY 2010 and 2012; diagnosis code 55011 for CMS FY 2013). The FY 2010 release contained a flat file with the corrections, but the ICD diagnosis codes in it lacked any leading zeros (e.g., the first two diagnosis codes are "10" and "11" instead of "0010" and "0011", which already require special handling to import and map back to what the values should be).
  * File format available in each subdirectory (the SPs import them):
	  * .\ICD_9: one file, bar-delimited format, with both short and long descriptions,
	  	with double-quotes surrounding every field
	  * .\ICD_10: one file, fixed-width format, with both short and long descriptions. The fixed-width format means that no delimiters exist. To account for that, the .fmt file has placeholders (Filler_##) for them, but does not load them to the permanent output table
...
| CMS FY | Source File on [ICD-9-CM Diagnosis and Procedure Codes: Abbreviated and Full Code Titles](https://www.cms.gov/Medicare/Coding/ICD9ProviderDiagnosticCodes/codes.html) | CMS ICD-9 Description Files for Diagnoses (DX) and Procedures (SG) |
| ------ | ----------- | ------------     |
