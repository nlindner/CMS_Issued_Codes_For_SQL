...
/* ======================================================================================
	AUTHOR: Nicole M. Lindner-Miles
	STORED PROCEDURE: ICD_Diag_002_Import_POA_Exempt_OPENROWSET.sql

	TODO: Document

	PURPOSE:
		*  Import MCE simple code exclusions from  this GitHub repo to a SQL Server table.
		*  This doesn't do much by itself, but the imported data can then be used to
			add MCE exclusion information to an existing ICD Diagnosis code 
			lookup/reference/dimension table (see subdirectory CMS_ICD_Code_Descriptions
			and the ICD##_Diag_000_Import_Descriptions... stored procedures for a working
			example of importing the ICD-9-CM and ICD-10-CM (diagnosis) code descriptions
			provided by CMS, if you do not already have one available).

	NOTES:
		*	CMS only provides spreadsheets with all POA-exempt ICD diagnosis codes for ICD-10-CM
			And the 2015 file has a different format than 2016-2018, so there are
			two format files within thsi directory
	EXAMPLE CALL:
...
====================================================================================== */
	...
	RAISERROR ('%s--- Create lookup table (to use in loop) with CMS fiscal years and the available source files with ICD-10-CM (diagnosis) codes that are exempt from POA reporting ---', 0, 1, @NewLine) 
	...
	/* Updates to clean up known issues from source file
	 	There are tabs (ASCII character 9) in the source files after the description ends.
