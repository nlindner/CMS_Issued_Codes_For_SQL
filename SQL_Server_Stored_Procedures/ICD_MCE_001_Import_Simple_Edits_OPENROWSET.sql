...
/* ======================================================================================
	...
	DESCRIPTION
		*	See repo LICENSE and see repo README for details on what the MCE is and 
		...
		*	Loads table with the single-code ICD edits from the CMS releases for 
		...
		*	The imported data can then be used to add MCE edit information to 
			...
			*	Within this repo, see subdirectories CMS_ICD_Code_Descriptions
				and the ICD_Diag_001_Import_Descriptions... and 
				ICD_Proc_001_Import_Descriptions stored procedures for working examples 
				of importing the ICD-9 and ICD-10 diagnosis and procedure code 
	...
	WHAT THIS PROCEDURE DOES
		...
		*	Load all ICD diagnosis codes and their descriptions:
			*	Create temporary table #CMS_FY_Release_Map and populate it, conditional on 
				...
				the CMS release info (ICD version, CMS FY, and effective/end dates) 
				for each file
	...
	EXAMPLE CALL
		exec dbo.ICD_MCE_001_Import_Simple_Code_Edits_OPENROWSET 
			...
			@Source_File_Dir = 'C:\Users\Nicole\Documents\GitHub\CMS_MS_DRG_Grouper_Help\CMS_MCE_Simple_ICD_Code_Edits'

	CHANGE LOG
		2018.02.25 NLM
			*	Redeploy this with changed file naming conventions
				(renamed from MCE_001_Import_Exclusions... to ICD_MCE_001_Import_Simple_Code_Edits)
				and relabeled from "Code Exclusions" to "Code Edits" to match CMS nomenclature
			*	Refactored the procedure to align similar code logic between this
				and the ICD_(Diag|Proc)_001_Import_Descriptions SPs and to add
				input parameters from other SPs (that is, @Table_Action, @Which_ICD_Version)
			...
			*	Fixed boneheaded mistake: to join between the output table here and in
				...
				to the final output table. While I'm add it, adding effective/end dates too.
			*	Also, because the MCE version is MCE-specific (it does not match the DRG 
...
====================================================================================== */
	...
	*/
	...
	/* Load all single ICD codes noted in MCE
	=================================================================================== */
	...
	BEGIN
		...
		SELECT 
			...
			,ICD_Code
			,ICD_Code_Desc
			--,CAST(MCE_Version AS INT) AS MCE_Version
			,MCE_Section
			,MCE_Edit_Type 
		FROM OPENROWSET(
